...
struct LorentzVector[dtype: DType = DType.float64](
    ...
    fn __init__(
        inout self, inout vector3d: Vector3D[dtype], t: Scalar[dtype]
    ) raises:
        self._size = 4
        self._ptr = DTypePointer[dtype].alloc(self._size)
    ...
    fn _elementwise_scalar_arithmetic[
        ...
        fn elemwise_vectorize[simd_width: Int](idx: Int) -> None:
            new_array._ptr.store[width=simd_width](
                idx,
                func[dtype, simd_width](
                    SIMD[dtype, simd_width](s),
                    self._ptr.load[width=simd_width](idx),
                ),
            )
    ...
    fn _elementwise_array_arithmetic[
        ...
        fn vectorized_arithmetic[simd_width: Int](index: Int) -> None:
            new_vec._ptr.store[width=simd_width](
                index,
                func[dtype, simd_width](
                    self._ptr.load[width=simd_width](index),
                    other._ptr.load[width=simd_width](index),
                ),
            )
    ...
    fn _elementwise_pow(self, p: Int) -> Self:
        ...
        fn tensor_scalar_vectorize[simd_width: Int](idx: Int) -> None:
            new_vec._ptr.store[width=simd_width](
                idx, math.pow(self._ptr.load[width=simd_width](idx), p)
            )
    ...
    fn tolist(inout self) -> List[Scalar[dtype]]:
        return List[Scalar[dtype]](
            self._ptr[0], self._ptr[1], self._ptr[2], self._ptr[3]
        )
    ...
    fn dot(inout self, other: Self) -> Scalar[dtype]:
        return (
            self.t() * other.t()
            - self._elementwise_array_arithmetic[mul](other)._reduce_sum()
        )
