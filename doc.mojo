...
fn main() raises:
    ...
    var vp = Vector3D[DType.float64].frompoint(
        x=0.0, y=3.0, z=4.0
    )  # Initializes a 3D vector with type=DType.float64 from given points x,y,z, v2 = [0.0, 3.0, 4.0]
    var v3 = Vector3D.fromvector(
        v2
    )  # Initializes a 3D vector using another 3D vector as argument, v3 =v2
    var v_sph = Vector3D.fromsphericalcoords(
        r=1.0, theta=0.5, phi=1.0
    )  # Initializes a 3D vector with given spherical coordinates
    var v_cyl = Vector3D.fromcylindricalcoodinates(
        rho=1.0, phi=1.0, z=3.0
    )  # Initializes a 3D vector with given cylindrical coordinates
    var v_fromlist = Vector3D.fromlist(
        List[DType.float64](3.0, 1.0, 2.0)
    )  # Initializes a 3D vector with given elements of List
    ...
    var v_cross = v1.cross(
        v2
    )  # does cross product of v1, v2 and returns the resultant vector
    ...
    v2.x(
        10.0
    )  # same as above, changes the x element to given value, similar syntax for y, z coodinates
    ...
    print(
        v1.isparallel(v2)
    )  # checks if v1 and v2 is parallel and returns Boolean (True or False)
    print(
        v1.isantiparallel(v2)
    )  # checks if v1 and v2 is anti parallel and returns Boolean (True or False)
    print(
        v1.isperpendicular(v2)
    )  # checks if v1 and v2 is perpendicular and returns Boolean (True or False)
